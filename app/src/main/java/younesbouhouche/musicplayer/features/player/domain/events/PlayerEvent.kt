...
sealed interface PlayerEvent {
    data class AddToQueue(val items: List<Long>) : PlayerEvent

    data class PlayNext(val items: List<Long>) : PlayerEvent

    data class Play(val tracks: List<Long>, val index: Int = 0, val shuffle: Boolean = false) : PlayerEvent
